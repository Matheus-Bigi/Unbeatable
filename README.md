...
- A commit also requires the hand to have actually stopped moving (`commitStabilityGate`, from the same motion-stability signal that smooths the prediction) — not just that the arm window is open. Without this, the engine could lock onto a still-forming, not-yet-final hand shape the instant "1" hit, which read as a coin flip (confidently wrong about as often as right). Committing only once the hand settles is what should make the Machine genuinely hard to beat, while a player who settles into their shape quickly still gets a legitimately early, snappy read. The stability signal tracks both wrist *and* per-fingertip velocity (`motionAnalyzer.js`) — wrist alone missed people who hold their forearm still and only move their fingers to shape the gesture; and fingertip motion is averaged per-tip displacement rather than the centroid of all 5 tips, since fingers curling inward symmetrically (a very common motion here) can leave their *average position* nearly fixed even while every tip is clearly still moving.
- The reaction-time number shown on the round banner is measured from "1" (`oneMs`), not from "GO" — it used to be measured from GO with a 10ms floor, which meant every commit anywhere in the (large, legitimate) pre-GO portion of the arm window displayed as the identical "0.010s", hiding all real variation in when the Machine actually locked in.
